...
db = MySQLdb.connect('localhost','root','','melanoma')
c = db.cursor()
...
def CustomerSearchDoctor(request):
    ...
    if request.POST:
        ...
        c.execute("select * from doctor_reg where hos_name = '"+a+"' and status='"+str(stat)+"'")
        data1 = c.fetchall()
    if request.GET:
    ...
    if request.POST:
        ...
        if request.FILES.get("file"):
            ...
            for item in filedata:
                print(item.name)
            try:
                original = cv2.imread('MelanomaApp/static/'+uploaded_file_url)
                contrast = cv2.imread(os.path.join(BASE_DIR,'images/dermquest/')+filename)
                original = cv2.cvtColor(original, cv2.COLOR_BGR2GRAY)
                contrast = cv2.cvtColor(contrast, cv2.COLOR_BGR2GRAY)
                ss=compare_images(original, contrast, "Original vs. Contrast")
                msg="Detected plz consult a doctor"
            except:
                msg="not detected"
            # if ss==1 :
            #     break
